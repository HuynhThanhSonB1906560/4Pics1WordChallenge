...
import vn.tutorials.a4pics1wordchallenge.impl.game.ModelImpl
import vn.tutorials.a4pics1wordchallenge.impl.game.PresenterImpl
import vn.tutorials.a4pics1wordchallenge.utils.MySharedPreference
import com.daimajia.androidanimations.library.Techniques
import com.daimajia.androidanimations.library.YoYo
import kotlinx.android.synthetic.main.back_dialog.view.no
...
class GameScreen : Fragment(R.layout.game_screen),
    ...
    override fun nameState(namePlayer: String?) {
    ...
    override fun backDialog() {
        val dialog = AlertDialog.Builder(requireContext()).create()
        val dialogBinding =
            LayoutInflater.from(requireContext()).inflate(R.layout.back_dialog, null, false)
        dialog.setCancelable(false)
        dialog.getWindow()?.setBackgroundDrawableResource(android.R.color.transparent);


        dialogBinding.yes.setOnClickListener { it ->
            (presenter as PresenterImpl).onClickYesBackButton()
            findNavController().navigate(R.id.action_gameScreen_to_mainScreen)

            dialog.cancel()
        }

        dialogBinding.no.setOnClickListener { it ->
            dialog.cancel()
        }
...
//        Toast.makeText(requireContext(), "Time's up!", Toast.LENGTH_SHORT).show()
        val dialog = AlertDialog.Builder(requireContext()).create()
        val dialogBinding =
            LayoutInflater.from(requireContext()).inflate(R.layout.timeout_dialog, null, false)
        dialog.setCancelable(false)
        dialog.getWindow()?.setBackgroundDrawableResource(android.R.color.transparent);
